...
- 表のセルの `title`（ホバー、スマホはタップで吹き出し）: 企業名は社名全体とティッカー、組入比率は
  前日の比率と差（pt）、時価総額はドル建て、株価は前日終値（差し替えられなかった行は保有ファイルの
  株価である旨）、前日比は変化額

### 前日比と NEW

前の日の CSV があれば、`symbol` を突き合わせて順位の差と前日の組入比率を出す。正なら順位が上がった。
