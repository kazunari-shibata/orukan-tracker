...
オルカン（eMAXIS Slim 全世界株式（オール・カントリー））の構成銘柄を、毎日まとめて確認するための
個人用のページ。公式の運用レポートは月1回・上位10社しか載らないので、全約2,200銘柄の
その日の組入比率の推計を自分で出している。

ページには次のものを載せている。

- 基準価額の推移（期間を切り替えられるグラフ）と前日比
- 国・地域別、業種別の構成比
- 全構成銘柄の一覧（組入比率・時価総額・株価・前日比・業種・国。銘柄名での検索、国での絞り込み、順位の上げ下げ）

組入比率は、iShares MSCI ACWI ETF の保有銘柄ファイルを、その日の株価と為替で計算し直した**推計**で、
公式の値ではない。

## 個人利用に留める

データ元（iShares / BlackRock の保有銘柄ファイル、Yahoo Finance の株価）の利用規約は、
個人・非商用の利用を前提にしていて、再配布や公開目的での利用を認めていない。
そのため、このページは**自分で見るためだけに使う**。

- URL を人に教えたり、SNS などに貼ったりしない
- 広告やアフィリエイトを載せない
- 生成したデータ（推計の CSV・保有銘柄ファイル・基準価額）はリポジトリにも Releases にも保存しない

このリポジトリに入っているのはコード・ページのひな形・ロゴだけで、データは含まない。

## 検索エンジンには載せない（noindex）

ページは GitHub Pages（https://kazunari-shibata.github.io/orukan-tracker/）に出しているが、
`<meta name="robots" content="noindex, nofollow">` を付けて、検索結果に出ないようにしている。
noindex は検索に載らないようにするだけで、アクセス制限ではない。URL を知っていれば誰でも開けるので、
上のとおり URL を広めないことで個人利用に留めている。
